...
const STORYTELLER_AVATAR = "default_storyteller.webp";

const storytellerAvatarUrl = (avatar) => {
  const image = String(avatar || STORYTELLER_AVATAR);
  if (isDefaultPlayerAvatar(image)) {
    return "/avatars/" + STORYTELLER_AVATAR;
  }
  if (
  ...
  return "/avatars/" + image.split("/").pop();
};
...
const mutations = {
  ...
  setStorytellerFabled(state, { name, image } = {}) {
    const storyteller = state.fabled.find((role) => role.id === "storyteller");
    if (!storyteller) return;

    if (name !== undefined) Vue.set(storyteller, "name", name);
    if (image !== undefined)
      Vue.set(storyteller, "image", storytellerAvatarUrl(image));
  },
