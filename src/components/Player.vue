      ...
      :class="[
        {
          ...
          'vote-own': isOwnVoteVisible,
          'vote-self': canSelfVote,
          'vote-unknown': isVoteUnknown,
        },
        player.role.team,
      ]"
    >
      ...
      <Token
        ...
        :id="player.id"
        :image="player.image"
        @set-role="handleTokenClick"
      />
      <button
        v-if="isVoteTokenActive"
        type="button"
        class="vote-token-hitbox"
        title="切换投票选择"
        @click.stop="vote()"
      ></button>
      <div
      ...
      <div class="overlay">
        ...
        <font-awesome-icon
          ...
          class="nominate"
          :class="{ warning: nominationTargetWarning }"
        ...
        >
          <template v-if="!session.isSpectator">
            ...
            <template v-if="!session.nomination">
              <li
                v-if="phaseInfo.isDay"
                @click="nominatePlayer()"
                :class="{ warning: nominationMenuWarning }"
              >
                <font-awesome-icon icon="hand-point-right" />
                提名
              </li>
              <li
                v-if="phaseInfo.isDay && hasNominatedToday"
                @click="clearNominatedByPlayer"
              >
                <font-awesome-icon icon="eraser" />
                清除已提名
              </li>
              <li
                v-if="phaseInfo.isDay && hasBeenNominatedToday"
                @click="clearNominatedPlayer"
              >
                <font-awesome-icon icon="eraser" />
                清除已被提名
              </li>
          ...
          <li
            ...
            :class="{
              disabled: player.id && player.id !== session.playerId,
            }"
          >
            <font-awesome-icon icon="chair" />
...
export default {
  ...
  computed: {
    ...
    isVoteTokenActive: function () {
      ...
      if (this.session.isSpectator) return this.canSelfVote;
      return this.voteLocked;
    },
    nominationKey: function () {
      return this.player && this.player.id
        ? `player:${this.player.id}`
        : `seat:${this.index + 1}`;
    },
    nominationDayKey: function () {
      return String(this.phaseInfo.day);
    },
    hasNominatedToday: function () {
      const nominations =
        this.session.nominationNominatorsByDay[this.nominationDayKey] || [];
      return nominations.includes(this.nominationKey);
    },
    hasBeenNominatedToday: function () {
      const nominations =
        this.session.nominationNomineesByDay[this.nominationDayKey] || [];
      return nominations.includes(this.nominationKey);
    },
    nominationMenuWarning: function () {
      return this.player.isDead || this.hasNominatedToday;
    },
    nominationTargetWarning: function () {
      return this.isSelectingNominee && this.hasBeenNominatedToday;
    },
    isOwnVoteVisible: function () {
      const session = this.session;
      if (!session.isSpectator) return false;
      if (!session.isSecretVote || !session.nomination) return false;
      if (!session.isVoteInProgress) return false;
      if (this.player.id !== session.playerId) return false;
      const nominee = this.players[session.nomination[1]];
      return !!nominee && nominee.role.team !== "traveler";
    },
    isVoteUnknown: function () {
      const session = this.session;
      if (!session.isSpectator) return false;
      if (!session.isSecretVote || !session.nomination) return false;
      if (!session.isVoteInProgress) return false;
  ...
  methods: {
    ...
    clearNominatedByPlayer() {
      this.isMenuOpen = false;
      this.$emit("trigger", ["clearNominatedByPlayer"]);
    },
    clearNominatedPlayer() {
      this.isMenuOpen = false;
      this.$emit("trigger", ["clearNominatedPlayer"]);
    },
    cancel() {
     ...
     */
    vote() {
      if (this.session.isSpectator && !this.canSelfVote) return;
      if (!this.session.isSpectator && !this.voteLocked) return;
      const nominee = this.session.nomination
        ? this.players[this.session.nomination[1]]
        : null;
      const voteWeight =
        nominee && nominee.role.team === "traveler"
          ? 1
          : Math.max(1, Number(this.player.votes) || 1);
      this.$store.commit("session/voteSync", [
        this.index,
        this.session.votes[this.index] > 0 ? 0 : voteWeight,
      ]);
    },
    addVote(player) {
      if (this.session.isSpectator) return;
...
// the seated player can mark their own current vote
#townsquare.vote.spectator
  .player.vote-own:not(.vote-yes)
  .overlay
  svg.vote.fa-times,
#townsquare.vote.spectator
  .player.vote-own.vote-yes
  .overlay
  svg.vote.fa-hand-paper,
#townsquare.vote.spectator
  .player.vote-self:not(.vote-yes)
  .overlay
  svg.vote.fa-times,
#townsquare.vote.spectator
  .player.vote-self.vote-yes
  .overlay
  svg.vote.fa-hand-paper {
  opacity: 1;
  transform: scale(1);
...
#townsquare.vote.spectator .player.vote-unknown .overlay svg.vote,
#townsquare.vote.spectator
  .player.vote-unknown.vote-lock.vote-yes
  .overlay
  svg.vote.fa-hand-paper,
#townsquare.vote.spectator
  .player.vote-unknown.vote-lock:not(.vote-yes)
  .overlay
  svg.vote.fa-times {
  opacity: 0;
  transform: scale(0.2);
...
#townsquare.vote.spectator .player.vote-unknown .overlay svg.vote.vote-question,
#townsquare.vote.spectator
  .player.vote-unknown.vote-lock
  .overlay
  svg.vote.vote-question {
  opacity: 1;
  transform: scale(1);
}
